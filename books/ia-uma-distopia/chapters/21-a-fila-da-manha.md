---
title: A fila da manhã
part: III — A NOTIFICAÇÃO
title_en: The Morning Queue
pov: Voss
when: '2047-10-14'
where: a mesa
premise: quarenta casos numa manhã, cada um com quatro linhas e um despacho; o capítulo
  passa rápido de propósito, e o desconforto do leitor é a forma. **No meio da fila
  entra um pedido de outra jurisdição, de muito longe, e o formulário é idêntico**
  — mesmo cabeçalho, mesma estrutura de protocolo, mesmos quarenta dias úteis. Voss
  não acha isso digno de nota. É o único lugar do livro que diz que o sistema é
  planetário, e diz em quatro linhas.
turn: ele bate a meta antes do almoço.
threads:
- fio-da-excecao
cast:
- voss
status: outline
---
